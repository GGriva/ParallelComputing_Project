...
int lower_bound_pred = 0;
int upper_bound_pred = 0;
int lower_bound_prey = 0;
int upper_bound_prey = 0;
int grass_init = 0;
...
{ 
  ...
  SV->Predator = tw_rand_integer(lp->rng, lower_bound_pred, upper_bound_pred);
  SV->Prey = tw_rand_integer(lp->rng, lower_bound_prey, upper_bound_prey);
  SV->Grass = grass_init;
  SV->Predator_in = 0;
  SV->Predator_out = 0;
  SV->Prey_in = 0;
...
int main(int argc, char **argv)
{
  ...
  //Input Variables
  sscanf(argv[1], "%d", &lower_bound_pred);
  sscanf(argv[2], "%d", &upper_bound_pred);
  sscanf(argv[3], "%d", &lower_bound_prey);
  sscanf(argv[4], "%d", &upper_bound_prey);
  sscanf(argv[5], "%d", &grass_init);
